...
function load_data(path::String,perc::Float64,batch::Int64,type::String)
    ...
    X = [(Array{Float32}(undef, 224, 224,3,batch)) for i=1:n_batch]
    Y = [(Array{String}(undef,batch)) for i=1:n_batch]
    if(n_add!=0)
        push!(X,(Array{Float32}(undef, 224, 224,3,n_add)))
    ...
    @time for leb in lebs
        ...
        for filename in names[1:n]
            idx = rng[i_rng] - 1
            i_rng +=1
            img = Images.imresize(Images.load(path*"\\"*type*"\\"*leb*"\\"*filename),224,224)
